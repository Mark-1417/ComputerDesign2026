...
module PLCPU(
	...
	wire        Less;        // ALU output less
	wire        Branch;      // ALU output branch
   ...
   // instantiation of control unit
	ctrl U_ctrl(
	    .Op(Op), .Funct7(Funct7), .Funct3(Funct3), .Zero(Zero), .Less(Less), 
		.RegWrite(RegWrite), .MemWrite(ID_MemWrite), .MemRead(ID_MemRead),
		.EXTOp(EXTOp), .ALUOp(ALUOp), .NPCOp(NPCOp), 
		.ALUSrc(ALUSrc), .WDSel(WDSel)
	);
 // instantiation of pc unit
	PC U_PC(.clk(~clk), .rst(reset), .en(~stall), .NPC(NPC), .PC(PC_out) );
	NPC U_NPC(.PC(PC_out), .NPCOp(EX_NPCOp),
	          .IMM(EX_immout), .JALR(aluout), .NPC(NPC));
	EXT U_EXT(
...
// instantiation of alu unit
	alu U_alu(.A(A), .B(B), .ALUOp(EX_ALUOp), .C(aluout), .Zero(Zero), .Less(Less), .Branch(Branch));
...
// MUX Gate 
	// 修改ALU输入源，加入旁路逻辑
	wire [31:0] alu_in1 = 
		(MEM_RegWrite && (MEM_rd != 0) && (MEM_rd == EX_rs1)) ? MEM_aluout :
		(WB_RegWrite && (WB_rd != 0) && (WB_rd == EX_rs1)) ? WD :
		EX_RD1;

	wire [31:0] alu_in2 = 
		(MEM_RegWrite && (MEM_rd != 0) && (MEM_rd == EX_rs2)) ? MEM_aluout :
		(WB_RegWrite && (WB_rd != 0) && (WB_rd == EX_rs2)) ? WD :
		EX_RD2;

	// 修改MEM阶段的存储数据旁路（WB->MEM）
	always @(*) begin
		if (WB_RegWrite && (WB_rd != 0) && (WB_rd == MEM_rs2)) begin
			memdata_wr = WD;  // 使用WB阶段的旁路数据
		end else begin
			memdata_wr = MEM_RD2;  // 默认使用MEM阶段的数据
		end
	end
...
//-----pipe registers--------------
	wire flush = (EX_NPCOp == `NPC_BRANCH) || (EX_NPCOp == `NPC_JUMP) || (EX_NPCOp == `NPC_JALR);
    ...
    pl_reg #(.WIDTH(64))
    IF_ID
    (.clk(~clk), .rst(reset), .flush(flush), .en(~stall), 
    ...
    assign EX_NPCOp = {ID_EX_out[154:151], ID_EX_out[150] & Branch};
    ...
    pl_reg #(.WIDTH(194))
    ID_EX
    (.clk(~clk), .rst(reset), .flush(flush), .en(1), 
    .in(stall ? 194'b0 : ID_EX_in), .out(ID_EX_out));
    ...
    pl_reg #(.WIDTH(146))
    EX_MEM
    (.clk(~clk), .rst(reset), .flush(0), .en(1), 
